...
- **`GenUiConversation`**: The main facade and entry point. It manages the conversation loop, orchestrates the `A2uiMessageProcessor` and `ContentGenerator`, and handles the flow of messages.
...
- **`A2uiMessageProcessor`**: Manages the state of the dynamic UI, including active surfaces and the `DataModel`.
...
2.  **UI State Management Layer**: `lib/src/core/` (`A2uiMessageProcessor`, `ui_tools.dart`).
...
1.  Initializing a `A2uiMessageProcessor` with a `Catalog`.
...
- `lib/src/core/`: `A2uiMessageProcessor`, `GenUiConfiguration`, `ui_tools.dart`.

...
1. Initialize and use the `Conversation`, the primary facade for the package.
...
1. **Initialization**: A `Conversation` is created with a simple system prompt instructing it to act as a helpful assistant. It's configured with an `onSurfaceAdded` callback.
...
4. **AI Response**:
   - The `Conversation` sends the conversation history to the configured `ContentGenerator` (either `FirebaseAiContentGenerator` or `GoogleGenerativeAiContentGenerator`).
...
5. **UI Rendering**:
   - `Conversation` listens to the stream and processes the `A2uiMessage`s, invoking the appropriate callbacks like `onSurfaceAdded`.
   - The `_handleSurfaceAdded` callback adds a new message item to the list, containing the `surfaceId`.
   - The `ListView` rebuilds, and a `Surface` widget is rendered for the AI's message, dynamically building the UI based on the `UiDefinition` managed by `SurfaceController`.

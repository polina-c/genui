...
class GoogleSchemaAdapter {
  ...
  GoogleSchemaAdapterResult adapt(dsb.Schema schema) {
    _errors.clear();
    final google_ai.Schema? googleSchema = _adapt(schema, ['#']);
  ...
  google_ai.Schema? _adaptObject(dsb.Schema dsbSchema, List<String> path) {
    ...
    if (objectSchema.properties != null) {
      for (final MapEntry<String, dsb.Schema> entry
        ...
        final google_ai.Schema? adaptedProperty = _adapt(
          entry.value,
          propertyPath,
        );
  ...
  google_ai.Schema? _adaptArray(dsb.Schema dsbSchema, List<String> path) {
    ...
    final google_ai.Schema? adaptedItems = _adapt(listSchema.items!, itemsPath);
